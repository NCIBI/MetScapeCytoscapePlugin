...
public class NullNetworkTranslator extends DefaultNetworkTranslator {
    ...
    public NullNetworkTranslator(MetabolicNetwork sourceNetwork, NetworkData networkData)
    {
    	super(networkData, sourceNetwork, null);
    }
	...
	public CyNetwork doTranslate() {
        ...
        {
            getTranslatorForNode(node).doTranslate(node);
        }
		for (MetabolicEdge edge : sourceNetwork.getEdges())
        {
            getTranslatorForEdge(edge).doTranslate(edge);
        }
