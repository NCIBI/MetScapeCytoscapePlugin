...
public class EnzymeNodeTranslator extends AbstractNodeTranslator implements NodeTranslator
{
    ...
    protected String getCanonicalName(MetabolicNode node) {
    	return EnzymeNode.getName(node);
    }

    @Override
    protected void addAttributes(String id, MetabolicNode node)
    {
    	Attributes.node.setAttribute(id, EnzymeAttribute.ECNUM.toAttributeName(), EnzymeNode.getEcnum(node));
        Attributes.node.setAttribute(id, EnzymeAttribute.NAME.toAttributeName(), EnzymeNode.getName(node));
    }
}
